...
func (app *application) routes() http.Handler {
	...
	router.Handle("/users/{id:[0-9]+}", authMiddleware.ThenFunc(app.getUserByID)).Methods("GET")

	router.Handle("/folders", authMiddleware.ThenFunc(app.getFolders)).Methods("GET")
	router.Handle("/folders/{id:[0-9]+}", authMiddleware.ThenFunc(app.getFolderByID)).Methods("GET")
	router.Handle("/folders", authMiddleware.ThenFunc(app.createFolder)).Methods("POST")
